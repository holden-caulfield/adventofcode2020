...
const digits = numDigits => value => value.match(new RegExp(`^\\d{${numDigits}}$`)) !== null
const range = (min, max) => value => parseInt(value) >= min && parseInt(value) <= max
...
const isHeight = value => {
    ...
    switch (unit) {
        case "cm": return range(150, 193)(amount)
        case "in": return range(59, 76)(amount)
...
const validatePassport = passport => Object.keys(validations).every(
    key => Object.keys(passport).includes(key) && validateField(key, passport[key])
)
